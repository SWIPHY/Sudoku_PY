...
def print_sudoku(s):
    ...
    """
    for row in range(9):
        for collone in range(9):
            print(s[row][collone], end=' ')
            if collone+1 == 3 or collone+1 == 6:
                print(" | ", end=' ')
...
def test_cell(s, row, collone):
    """
    Given a Sudoku puzzle s, row, and colloneumn number, return a list which represents
    ...
    for m in range(3):
        for n in range(3):
            used[s[m + block_row*3][n + block_collone*3]] = 1
        ...
        for row in range(9):
            for collone in range(9):
                ...
                for m in range(1, 10):
                    ...
                    if s[row][collone] == 0 and used[m] == 0:
                        s[row][collone] = m
                        stuck = False
                        break

def DFS_solve(s, row, collone):
    """
    ...
    return DFS_solve(s, row, collone+1)
